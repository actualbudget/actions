export class Trafico {
  ...
  static get STATUS () {
    return Object.freeze({
      ...
      NEEDS_MORE_APPROVALS: 'needsMoreApprovals',
      APPROVED: 'approved',
  ...
  async runTrafico (context, pullRequest, config) {
    ...
    statuses.forEach((status) => {
      switch (status) {
        ...
        case Trafico.STATUS.NEEDS_MORE_APPROVALS:
    ...
    if (this.config.reviewers) {
      ...
      if (!statuses || !statuses.length || statuses[0] !== Trafico.STATUS.WIP) {
  ...
  async _getStatuses () {
    const wipRegex = new RegExp(this.config.wipRegex, 'g')

    if (this.config.addWipLabel && wipRegex.test(this.pullRequest.title)) {
    ...
    if (reviews.length === 0) {
      ...
      if (changeRequestedReviews.length > 0) {
        ...
        if (approvedReviews.length < requiredApprovals) {
          statuses.push(Trafico.STATUS.NEEDS_MORE_APPROVALS)
          statuses.push(Trafico.STATUS.UNREVIEWED)
        } else {
          statuses.push(Trafico.STATUS.APPROVED)
        }
  ...
  async _getUniqueReviews () {
    ...
    const reviews =
      (
        await this.github.pulls.listReviews({
          owner,
          repo,
          pull_number: number
        })
  ...
  async _getReviewRequests () {
    ...
    return (
      await this.github.pulls.listRequestedReviewers({
        owner,
        repo,
        pull_number: number
      })
